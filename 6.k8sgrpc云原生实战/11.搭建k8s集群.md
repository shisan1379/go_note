...
# 向集合中添加一个条目
sudo ipset add test_set 192.168.1.0/24
# 测试条目是否在集合中
sudo ipset test test_set 192.168.1.1
# 列出集合内容
sudo ipset list test_set
# 最后清理删除测试集合
sudo ipset destroy test_set
```
注意：如果在此过程中遇到类似 Kernel error received: Invalid argument 或 Protocol not available 的错误，通常表明内核不支持 ipset 或特定的集合类型。
...
```bash
# apiserver-advertise-address   指定 API Server 监听地址 
# pod-network-cidr              与 Calico 网络插件匹配
sudo kubeadm init  --apiserver-advertise-address=192.168.4.101 --pod-network-cidr=10.244.0.0/16
```
...
# 单节点部署参照
<https://www.doubao.com/chat/12296526919254530>