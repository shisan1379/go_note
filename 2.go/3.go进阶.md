...
3. 使用完对象后放回池中：
    ...
    ```
sync.Pool 的大小是怎么控制的？
> 容量是内存的上限（当 NEW 新建时，发现连接池不能存储，会新建一个连接池）

sync.Pool 的容量什么时候回收呢？
> 会在 go 程序触发垃圾回收时，将全部资源清空，但是会在获取时，通过 New 方法 新建对象（对于新建开销很大的对象，不适合使用 sync.Pool） 。

sync.Pool是如何做到无锁操作，却并发安全的呢？
